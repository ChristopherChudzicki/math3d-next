...
Dedicated Cloudflare Worker that renders a per-scene Open Graph image for each
shared math3d scene and caches it in R2. It is **intentionally isolated and
abandonable**: it imports nothing from the rest of the monorepo (only
`@cloudflare/puppeteer` and its own relative modules), and nothing in the
monorepo imports it. Its sole coupling to the app is one var-gated block in the
app Worker.
...
- **R2 bucket** `math3d-og-images` (bound as `OG_BUCKET`).
- **Browser Rendering** entitlement enabled (bound as `BROWSER`).
- **R2 lifecycle rule** expiring the `og/lock/` prefix after ~1 day. This is a
  backstop only — the lock is self-healing (a stale lock older than ~2 min is
  taken over, see `lock.ts`) — but the rule bounds worst-case lock buildup and
  should still exist.
...
The app Worker only points `og:image` at this Worker when `OG_RENDER_ORIGIN` is
...
smoke-test it, then set `OG_RENDER_ORIGIN` to its `*.workers.dev` host and
redeploy the app Worker.

CI deploys this Worker in its own `deploy-og-render` job
...
1. Remove `OG_RENDER_ORIGIN` from `packages/app/wrangler.jsonc` (if set) and
   redeploy the app Worker — the app reverts to the static default card.
2. Delete the `deploy-og-render` job from
   `.github/workflows/deploy-reusable.yml`.
3. `wrangler delete` the `math3d-og-render` Worker.
4. Delete the R2 bucket `math3d-og-images` and its lifecycle rule.
5. Narrow `CLOUDFLARE_API_TOKEN` back to Workers-only scopes.
6. Delete this package (`packages/og-render`). Nothing else imports it, so no
   other code changes are needed.

## Development

```bash
yarn workspace og-render test        # vitest under @cloudflare/vitest-pool-workers
yarn workspace og-render typecheck
yarn wrangler dev                     # from packages/og-render (needs nodejs_compat)
```
