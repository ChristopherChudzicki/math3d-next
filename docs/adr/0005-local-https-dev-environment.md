...
[ADR-0004](0004-oauth-only-authentication.md) put sign-in behind Google and recorded that the flow cannot be exercised on the hostnames development uses: Google requires the host's TLD to be on the public suffix list and requires HTTPS outside bare `localhost`,[^origins] so `http://api.math3d.localdev:8000` fails on both counts. It named the two ways out — both servers onto `localhost`, or TLS terminated locally for a domain math3d owns — and chose neither, since neither is needed to develop against `dummy`. This ADR chooses.
...
Bare `localhost` needs no infrastructure and no code.[^localhost-free] What it leaves is a _mode_: a configuration entered to test Google and left to do anything else, alongside the `.localdev` hostnames daily development and the E2E suite use. The alternative is to stop having two hostname sets at all. That is [the design below](#the-rejected-design).
...
The design below is sound, and its benefit is real: one set of hostnames shared by daily development, the E2E suite, and a real sign-in, instead of a mode someone has to remember exists. What it cannot justify is its price against how rarely the need arises — a `mkcert -install` a human must run once per machine (an agent cannot), a Caddy container, two DNS records, a CA certificate threaded permanently into the E2E suite, a certificate that expires silently around two years out, and every existing worktree broken rather than degraded. Uniformity is worth paying for in proportion to how often the seam is crossed. This one is crossed a few sessions a year.
...
- **Google testing passes a few sessions a year** — most plausibly if a second provider lands and needs hand-checking on a cadence. That number is the threshold this decision rests on.
...
- **The flow needs testing from a phone,** or anything else that cannot install a root certificate. That reopens the question without arguing for this design — a [tunnel](#alternatives-considered) answers it better.
...
- **The switch is machine-wide, not local.** One Django container serves the main checkout and every worktree, and the development CORS origins are computed from `APP_BASE_URL`, so while `localhost` is in effect the worktree frontends on `math3d.localdev:3002`–`:3009` fall out of CORS and cannot read `csrftoken`. Tolerable only because the mode is entered deliberately and briefly.
- **Switching recreates the backend rather than restarting it.** A container's environment is fixed at creation, so it is `docker compose up -d`; Vite reads `APP_BASE_URL` at config load, so the dev server restarts too.
- **While `localhost` is in effect, development stops mirroring production's cookie topology.** `math3d.localdev` predates ADR-0004 — it arrived with the allauth migration (#1129) — and gives the SPA and API a shared registrable domain the way `next.math3d.org` and `api.next.math3d.org` do, which ADR-0004 records development as deliberately mirroring. Coverage of it rests on the E2E suite, which drives `dummy` through the same `provider/token` endpoint against real Django with the real `Domain=` attribute. Each half is exercised, never both in one run — and Django mints the same session cookie whichever provider produced the ID token.
...
Serve local development from `https://local.math3d.org:3000` (SPA) and `https://api.local.math3d.org:8000` (API), on a certificate from a local CA. It can be a single environment rather than a mode because the hostnames live under a domain math3d owns.[^zone-names]
...
`local.math3d.org` and `api.local.math3d.org` get A records pointing at `127.0.0.1`, DNS-only, in the Cloudflare zone math3d already owns — two explicit records, not a wildcard, since nothing needs a third name. They need matching AAAA records at `::1`; A records alone would work only where the resolver falls back cleanly.[^ipv6]

Owning the zone is what makes records cheaper than a hosts file — nothing to install, nothing to re-run on a new machine or in a new worktree, and no `sudo` step in `README.md`.[^ol-infra] Pointing a public name at loopback is well-trodden: `lvh.me` and `localtest.me` are the same trick on domains we do not control.
...
Let's Encrypt would issue over DNS-01, since the names are in a real zone, but that buys the whole ACME apparatus for a host that only answers to loopback.[^acme]
...
Caddy takes host `:8000` behind a compose profile, and `webserver`'s published port becomes `${WEBSERVER_HOST_PORT:-8000}` — the default unchanged, so a stack brought up without the profile still publishes Django where `.env.development` and CI expect it. Enabling HTTPS sets both `COMPOSE_PROFILES` and `WEBSERVER_HOST_PORT=8001` in the **gitignored project-root `.env`**, the only file compose interpolation reads.[^compose-env]
...
This is the one part right under either decision.
...
- **`.localdev` does not disappear even so** — CI keeps it, and it survives in the docs and as backend-test fixture literals.[^localdev-residue]

Two failure modes start cleanly and fail later:
...
- **The certificate expires silently.** A missing certificate fails the dev server loudly; an expired one starts fine and fails only in the browser, roughly two years out, with no reminder.

Neither records nor a hosts block make the dev servers reachable from another device — both bind loopback only. The records do, however, publish in a zone anyone can query that this setup exists.
...
[^zone-names]: `local.` rather than `dev.` only because `dev` is likelier to be wanted for something deployed. Both need confirming unused in the zone, which already carries `math3d.org`, `next.math3d.org`, and `api.next.math3d.org`.
[^ipv6]: Today's `/etc/hosts` block carries `::1` lines for both names, and `scripts/setup_worktree_env.sh` probes both loopbacks on the stated grounds that dev servers commonly bind only `::1`.
[^ol-infra]: `ol-infrastructure/local-dev` maintains a marker-delimited `/etc/hosts` block instead, because it defaults to `mit.dev`, a domain that team holds no zone for; a hosts file has no wildcards, so its eight hostnames are enumerated by hand.
...
[^localdev-residue]: `README.md`, `CLAUDE.md`, and the backend tests, which pass their own origins in explicitly — so they keep passing; they just stop resembling the configuration they describe.
