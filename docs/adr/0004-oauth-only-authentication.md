...
Sign-in yields an ID token carrying the user's identifier, email address and basic profile — all non-sensitive, so Google needs no verification review. allauth's own `email`/`profile` scope list is never sent: it is read only when building an authorization request, which the popup flow does not make.[^scopes] With a single provider, every returning user is matched on `uid`,[^uid-match] so there is no second identity to link and the question never arises.

Sign-in asks nothing beyond Google: one click, and no signup form is ever rendered. allauth routes a new identity whose address is already taken into `auth/provider/signup`,[^dupe-stage] which `SOCIALACCOUNT_ONLY` leaves mounted and whose form takes any unused address without comparing it to the provider's. `CustomSocialAccountAdapter.pre_social_login` refuses such a login outright, so the address on an account is always one the provider asserted.
...
**None of the redirect's scaffolding ships either.** Not the `allauth.urls` include that carries `OAuth2CallbackView`, not a registered redirect URI, not `GOOGLE_CLIENT_SECRET`, and not the `socialaccount_login_error` frontend URL. The first three serve the callback and nothing else: the token endpoint reverses no provider URL and verifies with `client_id` alone, the secret existing to exchange an authorization code this flow skips. The frontend URL is a near miss — its only reader is `on_authentication_error`, which `ProviderTokenView` never reaches, but `provider/redirect`'s form-error path does and falls back to `/`.[^token-path] Adding them alongside the callback later is a few lines; carrying them meanwhile means configuration that no code path reaches, which is the kind of thing that rots unnoticed and then fails when finally used. Google needs an _authorized JavaScript origin_ instead, which is a different field on the same client.
...
**`VITE_DISPLAY_AUTH_FLOWS` stays.** It is a separate question from authenticating with Google. The flag is presentation-only and one-sided: it hides the signed-out entry points and decides hamburger versus avatar for anonymous visitors (`UserMenu.tsx:28`), and the My Scenes gate is an OR (`ScenesListPage.tsx:38`), so a signed-in user keeps everything. `provider/token` stays mounted either way. Turning auth on in production is then flipping a GitHub Actions variable, an operational step decoupled from shipping the code — which matters when sign-in depends on a third-party script that may need hiding in a hurry. Stopping sign-in outright is `ENABLE_REGISTRATION`, and only for identities Google has not seen before. Retiring the flag is cleanup for later, once auth has been live long enough to be boring.
...
**No `AdminSite` override ships.** `/admin/` while signed out still renders Django's password form. `SOCIALACCOUNT_ONLY` does not disable it — allauth's `AuthenticationBackend` gates on `ACCOUNT_LOGIN_METHODS` and the password hash and never consults that setting — but no account is provisioned with a usable password, so nothing can satisfy the form as deployed; the way in is to sign in on the app first and come back. Replacing that form with an OAuth redirect is where the complexity lives, and it buys a convenience nobody needs: `admin_view` redirects _every_ failed `has_permission` to the login view, a signed-in non-staff user included,[^admin-redirect] so an override that bounces to the provider sends that user around the loop forever. It would also have to validate the `?next=` it is handed — `admin_view` puts `request.get_full_path()` there — or open a redirect that fires _after_ a session is established.

### User fields and deletion

`public_nickname` is dropped — the column, the profile form that edited it, and `PATCH /v1/auth/users/me/`, whose only writable field it was. It existed to label a user on publicly shared scenes, and under OAuth the only ways to fill it are to copy Google's real name in silently or to interrupt a one-click sign-in with a prompt for a value whose purpose the user cannot yet see.[^nickname] Constraint 4 rules out the first, and the second buys little: the field is not unique and nothing renders it today, so a user who never chose one costs nothing. A signed-in user is identified by their email address, shown only to themselves. The header trigger follows: a hamburger for anonymous visitors, and for a signed-in user a generic person avatar, since with no nickname and constraint 4 barring Google's profile data any initial would have to be derived from the email address.
...
- **Superuser bootstrap runs after the first sign-in, not before.** `createsuperuser` works, but the account it creates carries no `SocialAccount` row, and with linking off Google will never match it. Signing in afterwards with the same address collides on the unique email and is refused with `email_taken`. So the order is: sign in through Google first, then set `is_staff` and `is_superuser` on the row allauth created. That is a one-off `manage.py shell` line, not a management command — it runs once in production, and development gets its admin from `createsuperuser`, whose password still authenticates at `/admin/`.
- **Losing the Google account means losing the admin.** A password fallback exists mechanically — `createsuperuser` with a password still authenticates at `/admin/` — but none is provisioned, and provisioning one is the alternative rejected below. That is where ADR-0002's render budget lives (`scenes/admin.py`).
- **Deleting an account takes only the session.** `POST /v1/auth/users/me/delete/` has no re-authentication step — the password check it used to make cannot survive accounts that have no password — so a stolen session deletes the account, and orphans its scenes, in one request. The typed confirmation phrase is the only friction, and it is client-side.
...
[^signup-fields]: `ACCOUNT_SIGNUP_FIELDS = ["email*"]`. Trimming it flips the derived [`SOCIALACCOUNT_EMAIL_REQUIRED`](https://github.com/pennersr/django-allauth/blob/65.15.0/allauth/socialaccount/app_settings.py#L63-L71) to false, and that setting is the only thing stopping an ID token with no `email` claim from auto-signing-up an account with a blank address ([`flows/signup.py:83-87`](https://github.com/pennersr/django-allauth/blob/65.15.0/allauth/socialaccount/internal/flows/signup.py#L83-L87)).
...
[^origins]: [Google Cloud — Manage OAuth Clients](https://support.google.com/cloud/answer/15549257), on authorized JavaScript origins: the TLD must be on the public suffix list, and HTTPS is required outside `localhost`. The [redirect-URI rules](https://developers.google.com/identity/protocols/oauth2/web-server) impose the same two, so the redirect flow inherits the constraint unchanged.
...
[^signup-hook]: [`socialaccount/adapter.py:163`](https://github.com/pennersr/django-allauth/blob/65.15.0/allauth/socialaccount/adapter.py#L163) delegates to the account adapter's `is_open_for_signup`, which here is ours (`authentication/adapter.py:6-7`) returning `settings.ENABLE_REGISTRATION`.
...
[^scopes]: [`google/provider.py:61-65`](https://github.com/pennersr/django-allauth/blob/65.15.0/allauth/socialaccount/providers/google/provider.py#L61-L65) — `get_default_scope` is reached only through `OAuth2Provider.get_scope()`, which builds the authorization redirect. What allauth reads on this path is the ID token's own claims, via [`extract_email_addresses`](https://github.com/pennersr/django-allauth/blob/65.15.0/allauth/socialaccount/providers/google/provider.py#L85-L91).
[^signup-closed]: [`headless/socialaccount/views.py:108-109`](https://github.com/pennersr/django-allauth/blob/65.15.0/allauth/headless/socialaccount/views.py#L108-L109) turns `SignupClosedException` into `ForbiddenResponse`, whose body is `{"status": 403}` with no `errors` key. The `signup_closed` code is produced by [`headless/socialaccount/internal.py:65`](https://github.com/pennersr/django-allauth/blob/65.15.0/allauth/headless/socialaccount/internal.py#L65), on the callback path this deployment does not mount.
