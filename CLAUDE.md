...
2. The main checkout needs a gitignored `.env` with `VITE_DISPLAY_AUTH_FLOWS=true` and `ENABLE_REGISTRATION=True` (the committed `.env.development` defaults both off). `ENABLE_REGISTRATION` gates the entire suite, not just the signup tests: every ephemeral test user is a first-time provider identity, and `provider/token` returns 403 when signup is closed. `VITE_GOOGLE_CLIENT_ID` also needs to be set, but `.env.development` already ships a placeholder value that's sufficient for the suite — a real client ID is only needed to exercise Google sign-in by hand.
