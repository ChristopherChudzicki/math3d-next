...
2. The main checkout needs a gitignored `.env` with `VITE_DISPLAY_AUTH_FLOWS=true` (the committed `.env.development` defaults it off). The suite signs in through the dummy provider and needs nothing Google-specific; to exercise Google by hand, see README.md's "Testing Google sign-in locally".
